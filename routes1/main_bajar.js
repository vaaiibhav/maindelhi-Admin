...
router.post('/left_value', function (req, res, next) {
    ...
    async function sam() {
        ...
        await knex('main_bajar').select("*").where('date', date).then(async result_date => {
            ...
            if (result_date.length == 0) {
                status = 'true'
                await knex('main_bajar').insert({ value1: value1, left_no: value2, date: date,status:sta }).then(result => {
                    console.log('This is insert result arter installation ', result);
                    res.render('dashboard', { status })
                })
            } else {
                await knex('main_bajar').where('date',result_date[0].date ).update({  value1: value1, left_no: value2,status:sta }).then(result => {
                    res.render('dashboard',{status})
                })
            }
        })
...
router.post('/right_value', function (req, res, next) {
    ...
    var sta = 'true'
    const date = moment().tz('Asia/Kolkata').format('yyyy-MM-DD');
    knex('main_bajar').select("*").where('date', date).then(result_date => {
        status = 'true'
        knex('main_bajar').update({ value2: value1, right_no: value2,status:sta  }).where('date', date).then(result => {
...
router.get('/main_bajar', function (req, res, next) {
    var date = moment().tz('Asia/Kolkata').format('yyyy-MM-DD');
    knex('main_bajar').select('*').where('date', date).then(result => {
        if (result.length == 0) {
...
router.post('/old_data',function (req,res,next) {
    ...
    knex('main_bajar').select('*').where('date',date).then(result_date =>{
        if (result_date.length == 0) {
            status = "true"
            knex('main_bajar').insert({ value1: lv, left_no: l, value2:rv,right_no:r,date: date }).then(result => {
...
router.get('/set_res',function (req,res,next) {
    var status = req.query.result_btn;
        knex('main_bajar').update({status : status}).orderBy('id', 'desc').limit(1).then(result1=>{
